...
@app.route('/dummyimage', methods=['GET', 'POST'])
def dummy_image():
    acknowledge = {
        'updated': 'False'
    }
    ...
    try:
        if userType == 'admin':
            if imageDetails is not None:
                dummy_details = {
                    'userType': 'admin',
                    'imageDetails': imageDetails
                }
                acknowledge = dummy_services.insert(dummy_details=dummy_details)
    except:
        acknowledge['updated'] = 'False'
    return acknowledge

@app.route('/comparedummy', methods=['GET', 'POST'])
def compare_dummy():
    acknowledge = {
        'found': False,
        'message': 'Not Found'
    ...
    try:
        if userType == 'admin':
            if imageDetails is not None:
                ...
                acknowledge = dummy_services.compare_dummy_face(info=info)
    except:
        acknowledge['message'] = 'Error Occured'
    return acknowledge
